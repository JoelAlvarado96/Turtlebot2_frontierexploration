...
#Class that implements priority queue./ Clase que implementa cola de prioridad. 
class PriorityQueue:
...
#Class that implements grid. / Clase que implementa grid. 
class Grid:
    ...
    def __init__(self, data, height, width, resolution, origin):
        ...
        #convert 1 dimensional array to 2 dimensional array / convertir matriz de 1 dimensión en matriz de 2 dimensiones 
    ...
    #Gets the value of the cell from the grid / Obtiene el valor de la celda de la cuadrícula 
    ...
    #Checks if cell coordinate is valid / Comprueba si la coordenada de la celda es válida
    ...
    #Checks if a given cell is a tuple that consists of 2 elements / Comprueba si una celda determinada es una tupla que consta de 2 elementos 
    ...
    #Checks if a given cell is within the current grid / Comprueba si una celda determinada está dentro de la cuadrícula actual 
    ...
    #Abstract method that scales the map by the scaleFactor. / Método abstracto que escala el mapa por scaleFactor.
    def scale(self, scaleFactor):
        raise NotImplementedError("scale() method is not implemented!")

    #Populates the set with cells that have the given value / Rellena el conjunto con celdas que tienen el valor dado 
    @staticmethod
    def populateSetWithCelcurls(grid, set, value):
...
#Class that implements occupancy grid / Clase que implementa cuadrícula de ocupación 
class OccupancyGrid(Grid):
    ...
    #Gets neighbors of the specific cell / Obtiene vecinos de la celda específica 
    ...
    def expandObstacles(self):
        ...
        if not self.obstacles: #If the obstacle set is empty, then iterate through the entire map and find obstacles  / Si el conjunto de obstáculos está vacío, recorre todo el mapa y encuentra obstáculos. 
        ...
        if self.empty: #If the empty cell cache is not empty, then remove empty cells that turned into obstacles after expansion / Si la caché de celda vacía no está vacía, elimine las celdas vacías que se convirtieron en obstáculos después de la expansión 
            self.empty = self.empty - self.obstacles

    #Adds cost grid to allow local cost map processing / Agrega una cuadrícula de costos para permitir el procesamiento de mapas de costos locales 
    def addCostGrid(self, costGrid):
        ...
        #Warning: The offset calculation / Advertencia: el cálculo de compensación 
    ...
    #Gets heuristic relevant to the current grid / Obtiene heurística relevante para la cuadrícula actual 
...
class PathFinder:
    ...
    #Finds path from goal to destination using internal parent list / Encuentra la ruta de la meta al destino usando la lista de padres interna 
    def findPath(self):
...
#A class that has a function of cell type enumeration. / Una clase que tiene una función de enumeración del tipo de celda.
class CellType:
    Unexplored = -1
    Empty = 0
    Obstacle = 100

#Processes the received occupancy grid message. / Procesa el mensaje de cuadrícula de ocupación recibido. 
...
#Callback function that processes the initial position received. / Función de devolución de llamada que procesa la posición inicial recibida. 
...
#Creates a GridCells message and auto-initializes some fields. / Crea un mensaje GridCells y autoinicializa algunos campos. 
...
#Service function that calculates trajectory for the robot to follow in order to get to the goal cell. / Función de servicio que calcula la trayectoria que debe seguir el robot para llegar a la celda objetivo. 
...
if __name__ == '__main__':
    ...
    expanded_cell_pub = rospy.Publisher('/expandedGridCells', GridCells, queue_size=5) #celdas de cuadrícula expandidas. Publisher for commanding robot motion
    frontier_cell_pub = rospy.Publisher('/frontierGridCells', GridCells, queue_size=5) # celdas de cuadrícula de frontera. Publisher for commanding robot motion
    path_cell_pub = rospy.Publisher('/pathGridCells', GridCells, queue_size=5)#celdas de cuadrícula de ruta
    cluster_cell_pub = rospy.Publisher('/clusterGridCells', GridCells, queue_size=5)#clúster de celdas de cuadrícula
    centroid_cell_pub = rospy.Publisher('/centroidGridCell', GridCells, queue_size=5)#celdas de cuadrícula centroide
    empty_cell_pub = rospy.Publisher('/emptyGridCell', GridCells, queue_size=5)#celdas de cuadrícula vacías
    obstacle_cell_pub = rospy.Publisher('/obstacleGridCell', GridCells, queue_size=5)#celdas de cuadrícula de obstaculo
    unknown_cell_pub = rospy.Publisher('/unknownGridCell', GridCells, queue_size=5)#celdas de cuadrícula desconocida
